...
from fontaine.font import CharsetInfo, FontFactory
from fontaine.structures.dict2xml import dict2txt, dict2xml
...
db = os.environ.get("UNAMES_DB") or os.path.join(
    os.path.dirname(__file__), "charsets", "names.db", "en.names-db"
)
...
def format(x):
    if isinstance(x, str):
        return x
    return "U+{:04x} {} {}".format(x, chr(x), unicodedata.name(chr(x), ""))
...
def unicodevalues_asstring(values):
    ...
    return map(lambda x: f"U+{x:04x} {chr(x)}", sorted(values))


def extract_firstline(text):
    return (text or "").replace("\r", "\n").split("\n")[0]


class Director:
    def __init__(self, show_hilbert=None, charsets=[], missing=False, _library=None):
    ...
    def represent_coverage_png(self, font):
        ...
        for cmap in cmaps:
            ...
            filename = "{}/{}-{}-hilbert".format(
                self.output_directory, font.common_name, cmap.common_name
            )
            ...
            fp.close()
            hilbertScript = (
                'simpleHilbertCurve --outFormat=png --level=3 --out="%s" "%s"'
        ...
        for font_filename in fonts:
            ...
            for charsetinfo in font.get_orthographies(self.library):
                ...
                if charsetinfo.support_level != SUPPORT_LEVEL_FULL:
                    ...
                    orth["orthography"]["SetTotal"] = (
                        charsetinfo.glyphs_in_charset_count
                    )
            ...
            if fonts_charactersets_names:
                if (
                    tree["identical"]
                    and fonts_charactersets_names != font_charactersets_names
                ):
...
class CharMapGen:
    def __init__(self, chars, **kwargs):
        self.py_gen = PyGen()
        self._chars = chars
        self._common_name = kwargs.get("common_name", "")
        self._native_name = kwargs.get("native_name", "")
    ...
    def generate(self):
        ...
        self.py_gen.indent()
        self.py_gen.write(f"common_name = '{self._common_name}'")
        self.py_gen.write(f"native_name = '{self._native_name}'")
        self.py_gen.newline()
        ...
        for char in self._chars:
            ...
            self.py_gen.write(
                f"chars.append({hex_formatted})  #{char[1]}\t{char[2]}"
            )
...
class GlyphMapGen:
    ...
    def generate(self):
        ...
        for glyph in self._glyphs:
            ...
            self.py_gen.write(f"glyphs.append({hex_formatted})  #{glyph[0]}")
            ...
            for subset in _library.charsets:
                ...
                print(
                    "| [[ {} ]] ({}/{})  || style='text-align:right'".format(
                        subset.common_name,
                        len(glyphs) - len(list(charsetinfo.missing)),
                        len(glyphs),
                    ),
...
def pprint_dict(obj, indent, length):
    for i, key in enumerate(obj.keys()):
        ...
        if type(obj[key]) in [str, int]:
            ...
            sys.stdout.write(f'{indent}  {key!r}: "{value}"{comma}')
        else:
            sys.stdout.write(f"{indent}  {key!r}:")
...
def pprint(obj, indent="", items_length=0):
    ...
    if isinstance(obj, OrderedDict):
        ...
        if items_length > 0:
            comma = ", "
        sys.stdout.write(f"{indent}}}{comma}")
